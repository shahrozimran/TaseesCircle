...
export default function PakistanDiscussionsPage() {
  ...
  return (
    <>
      {/* Hero */}
      <Hero
        subtitle="🇵🇰 Pakistan Community Hub"
        title="Rizq e Halal & Knowledge Discussions"
        description="Authentic Islamic guidance on earning halal livelihood, evaluating job ethics, avoiding riba, freelancing, and Zakat in Pakistan — grounded in Quran and Sunnah."
          ...
          {filteredDiscussions.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
              {filteredDiscussions.map((disc, i) => (
                <DiscussionCard key={disc.slug} {...disc} community="pakistan" index={i} />
              ))}
            </div>
          ) : (

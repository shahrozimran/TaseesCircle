...
function StatusBadge({ status }) {
  ...
  return <span className={map[status] || map.open}>{labels[status] || status}</span>;
}
...
function PriorityBadge({ priority }) {
  ...
  return <span className={map[priority] || map.medium}>{priority || "medium"}</span>;
}
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-beige-200 p-12 text-center">
          ...
          <p className="text-sm text-charcoal-300">All caught up!</p>
        </div>
      ) : (
              ...
              <div className="p-5 sm:p-6">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-charcoal-300 flex items-center gap-1 mt-1 flex-wrap">
                      ...
                      {ticket.masjids?.name && <span className="ml-1">· {ticket.masjids.name}</span>}
                    </p>
                    <p className="text-[11px] text-charcoal-200 mt-0.5 flex items-center gap-1">
                      ...
                      <span className="ml-1">· To: <span className="font-medium">{ticket.recipient === "tasees_admin" ? "TaseesCircle Admin" : "Moderator"}</span></span>
                    </p>
                  </div>
                  ...
                  {ticket.status !== "closed" && (
                    <button
                      ...
                      title="Close ticket"
                      className="ml-3 p-1.5 text-charcoal-200 hover:text-red-400 hover:bg-red-50 rounded-lg transition-colors shrink-0"
                    >
                ...
                {ticket.ticket_responses?.length > 0 && (
                  <div className="space-y-2 mb-3">
                    <p className="text-[11px] font-bold text-charcoal-300 uppercase tracking-wide">
                      Responses ({ticket.ticket_responses.length})
                    </p>
                          ...
                          <div className="flex items-center gap-2 mb-2 flex-wrap">
                            ...
                            {resp.email_sent ? (
                              <span className="flex items-center gap-1 text-[10px] text-green-600 font-medium">
                                <MailCheck size={11} /> Email sent
                              </span>
                            ) : (
                              <span className="flex items-center gap-1 text-[10px] text-amber-500 font-medium">
                                <MailX size={11} /> Email pending
                              </span>
                            )}
                          </div>
                    ...
                    {respondingTo === ticket.id ? (
                      <div className="border-t border-beige-100 pt-4 mt-3">
                        ...
                        <textarea
                          ...
                          placeholder="Type your response... This will be sent to the user's email and shown in their dashboard."
                          rows={4}
                        ...
                        <div className="flex items-center justify-between mt-3 flex-wrap gap-2">
                          <p className="text-[11px] text-charcoal-200 flex items-center gap-1">
                            <Mail size={10} />
                            Response will be emailed to <strong className="ml-1">{ticket.profiles?.email}</strong>
                          </p>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        >
                          ...
                          {ticket.ticket_responses?.length > 0 ? "Reply Again" : "Respond"}
                        </button>
                        {ticket.status === "responded" && (
                          <span className="flex items-center gap-1 text-[11px] text-green-600 font-medium ml-1">
